#!/usr/bin/env bash
# Rebuilds the Inside the Mandala reel (v3). Run where ffmpeg, sox and python3 (Pillow, numpy) are installed.
# Motion rule: dome content only expands outward from the zenith. No rotation anywhere.
# Look: 2K AI-upscaled source, lanczos scaling, contrast adaptive sharpening, and a slow
# breathing glow (4 s cycle) so the mandala swells and settles like a breath.
set -e
...
curl -sf -o src.mp4 "$SRC2K"  # ByteDance 2K upscale of the original mandala footage
python3 mk.py
sox dry.wav bed.wav gain -8 reverb 70 50 100 100 0 0 gain -n -2 trim 0 21.6
V="scale=1080:-2:flags=lanczos,crop=1080:1920,fps=24,setsar=1,cas=0.5,eq=contrast=1.04:saturation=1.06"
G="vignette=PI/5,noise=alls=3:allf=t"
E="-c:v libx264 -crf 12 -preset fast -pix_fmt yuv420p -an"
# Breathing glow: blurred copy screened on top, strength swelling on a 4 s cycle
BREATH="format=gbrp,split[a][b];[b]gblur=sigma=18[gl];[a][gl]blend=all_expr='A+(0.28+0.18*sin(2*PI*T/4))*(B-A*B/255)',format=yuv420p"
...
# Real footage looking straight up, telescopic zoom outward from the zenith, breathing
pov(){ ffmpeg -v error -y -ss $1 -t $2 -i src.mp4 -loop 1 -t $2 -i grad.png -loop 1 -t $2 -i $3 -filter_complex "[0:v]crop=iw:iw:0:(ih-iw)/2,scale=w='trunc(1500*(1+0.06*t+0.018*sin(2*PI*t/4))/2)*2':h=-2:eval=frame:flags=lanczos,pad=1920:1920:(ow-iw)/2:(oh-ih)/2:black,crop=1080:1920,fps=24,setsar=1,cas=0.7,eq=saturation=1.12:contrast=1.06,$BREATH,$G[b0];[b0][1:v]overlay[g];[2:v]format=rgba,fade=t=in:st=$4:d=0.9:alpha=1,fade=t=out:st=$5:d=0.6:alpha=1[t];[g][t]overlay,format=yuv420p[v]" -map "[v]" -t $2 $E $6; }
...
# End card over dimmed real footage, crisp, slow outward zoom and breathing glow
ffmpeg -v error -y -ss 3.0 -t 7.0 -i src.mp4 -loop 1 -t 7.0 -i end.png -filter_complex "[0:v]crop=iw:iw:0:(ih-iw)/2,scale=w='trunc(1500*(1+0.03*t+0.012*sin(2*PI*t/4))/2)*2':h=-2:eval=frame:flags=lanczos,pad=1920:1920:(ow-iw)/2:(oh-ih)/2:black,crop=1080:1920,fps=24,setsar=1,cas=0.6,eq=saturation=1.1,$BREATH,colorchannelmixer=rr=0.55:gg=0.55:bb=0.55,vignette=PI/4[b0];[1:v]format=rgba,fade=t=in:st=0.5:d=1.0:alpha=1[t];[b0][t]overlay,format=yuv420p[v]" -map "[v]" -t 7.0 $E s5.mp4
